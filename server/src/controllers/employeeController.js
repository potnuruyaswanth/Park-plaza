...
export const markWorkAsDone = async (req, res) => {
  try {
    const employeeId = req.user.id;
    const { bookingId } = req.params;
    const { notes } = req.body;
    ...
    // Verify employee belongs to this booking's showroom
    const employee = await User.findById(employeeId);
    if (!employee.showroomId.equals(booking.showroomId._id)) {
    ...
    // Update all invoices associated with this booking to PAID
    const invoices = await Invoice.updateMany(
      { bookingId: booking._id },
      { status: INVOICE_STATUS.PAID }
    );

    // Update all payments associated with this booking to PAID
    const payments = await Payment.updateMany(
      { bookingId: booking._id },
      { status: 'PAID' }
    );

    // Restore the parking slot (increase availableSlots)
    const showroom = await Showroom.findById(booking.showroomId._id);
    if (showroom && showroom.availableSlots < showroom.totalParkingSlots) {
      showroom.availableSlots += 1;
      await showroom.save();
    }

    // Populate for response
    await booking.populate(['userId', 'showroomId', 'employeeId']);

    res.status(200).json({
      message: '✅ Work marked as done! Slot #' + booking.slotNumber + ' is now available.',
      booking,
      invoicesUpdated: invoices.modifiedCount,
      paymentsUpdated: payments.modifiedCount,
      slotsRestored: 1
    });
  } catch (error) {
